...
- When creating the password, uncheck the "User must change password at next log in" box before clicking "Next".
  This is for the sake convenience during this lab. In a real work application, you want the user to create thier own password.
...
In order to join the Domain we have to use the Domain Controller as the DSN server. This is because the Domain Controller knows what "mydomain.com" is. The current server will try to located the domina on the internet, as opposed to wthin the server with have configured. 
